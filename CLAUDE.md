...
- A user who sets `profiles.share_shelf = true` (migration 005, default `false`) exposes `title`, `author`, `cover_url`, `stage`, `is_reading` and `updated_at` for every one of their books — nothing else, no ids, notes, prices or `user_id` — through the `public_shelf` view, which `anon` and `authenticated` may `SELECT`. The view has no owner-side RLS of its own; its `WHERE p.share_shelf` clause is the entire boundary between "public" and "private", so widening the `SELECT` list or the join needs a review, not a quick edit. There is no in-app UI to flip the flag yet — set it per-user with `UPDATE public.profiles SET share_shelf = true WHERE id = '<uuid>';` in the Supabase dashboard SQL editor. Migrations here have no CLI workflow (the `supabase` CLI is not part of this project's toolchain): apply a new file by pasting it into the dashboard's SQL editor, in filename order
